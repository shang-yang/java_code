...
import com.shangyang.sorm.utils.JavaFileUtils;
import com.shangyang.sorm.utils.StringUtils;
		...
		try {
			...
			while(tableRet.next()) {
				...
				while(set2.next()) {
					...
					ti.getProKeys().add(ci2);
				}
				
				if(ti.getProKeys().size() > 0) {	//取唯一主键。。方便使用。如果是联合主键。则为空！
					ti.setOnlyPriKey(ti.getProKeys().get(0));
				}
			}
